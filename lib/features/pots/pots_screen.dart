...
class _PotsScreenState extends ConsumerState<PotsScreen> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final resp = await dio.get('/v1/mobile/pots');
      final data = resp.data as List? ?? [];
      setState(() {
        _items = data
            .map((e) => SavingPot.fromJson(Map<String, dynamic>.from(e)))
            .toList();
      });
  ...
  Future<void> _create() async {
    ...
    final dio = ref.read(apiProvider);
    try {
      await dio.post('/v1/mobile/pots',
          data: {'name': name, 'target_cents': target * 100});
      _nameCtrl.clear();
      _targetCtrl.clear();
      _fetch();
    } catch (_) {}
  ...
  Future<void> _add(String id, int rupees) async {
    final dio = ref.read(apiProvider);
    try {
      await dio.patch('/v1/mobile/pots/$id', data: {'add_cents': rupees * 100});
      _fetch();
    } catch (_) {}
  }
