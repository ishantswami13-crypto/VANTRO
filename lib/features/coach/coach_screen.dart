...
class _CoachScreenState extends ConsumerState<CoachScreen> {
  ...
  Future<void> _generate() async {
    ...
    try {
      final resp = await dio.post('/v1/mobile/coach/plan', data: {
        'income_cents': (int.tryParse(_incomeCtrl.text.trim()) ?? 0) * 100,
        'rent_cents': (int.tryParse(_rentCtrl.text.trim()) ?? 0) * 100,
        'goal': _goalCtrl.text.trim(),
      });
      setState(() {
        _plan = CoachPlan.fromJson(Map<String, dynamic>.from(resp.data));
      });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Week starting ${plan.weekStart}',
            style: const TextStyle(fontWeight: FontWeight.w700)),
        const SizedBox(height: 8),
        Text('Health Score: ${plan.healthScore}',
            style: const TextStyle(fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        const Text('Rules'),
        const SizedBox(height: 6),
        ...plan.rules.map(
            (r) => Row(children: [const Text('• '), Expanded(child: Text(r))])),
        const SizedBox(height: 8),
        Text('Nudge: ${plan.dailyNudge}',
